...
return (
	<section className=' flex size-full flex-col gap-8 text-white'>
			<div className={`${hour >= "21" ? 'bg-night' : (hour > "12" ? 'bg-afternoon' : 'bg-day')} h-[303px] flex flex-col justify-between  items-start rounded-xl w-full px-10 py-9`}>
			<div className='flex justify-between size-full  '>
				<div className='z-10 relative flex justify-between flex-col'>
					<h1 className= 'greet text-white text-6xl font-bold'>
					{ ( hour >= "21" ? "Good Night" : (hour > "12" ? "Good Afternoon" : "Good Day"))}
					</h1>
					<p className='text-2xl capitalize font-semibold '>{user?.firstName + " " + user?.lastName}</p>
				</div>
